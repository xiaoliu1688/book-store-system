...
public class UserServlet extends ModelBaseServlet {
    ...
    public void doLogin(HttpServletRequest request,HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            request.getSession().setAttribute(bookConstants.USERSESSIONKEY,user1);//将user1对象存入会话域
    ...
    public void checkUsername (HttpServletRequest request,HttpServletResponse response) {
        ...
        try {
            String username = request.getParameter("username");
            userService.findByUsername(username);//如果此处出现异常则表示用户已经存在
            //没出现异常说明用户用户不存在可以进行注册
            commonResult = CommonResult.ok();//flag为true
        } catch (Exception e) {
            e.printStackTrace();
            commonResult = CommonResult.error().setMessage(e.getMessage());

        }
        JsonUtils.writeResult(response,commonResult);//将描述请求处理信息的对象封装成json形式响应给axios
